...
try:
    ...
    st.error("❌ Missing secrets or config. Please define your credentials in `.streamlit/secrets.toml` or Streamlit Cloud settings.")
    st.stop()

client = OpenAI(
    api_key=config["api_key"],
    base_url=config.get("api_base", None),
    default_headers={"api-key": config["api_key"]} if config["provider"] == "azure" else None,
)
                ...
                with st.spinner("Summarizing..."):
                    try:
                        res = client.chat.completions.create(
                            model="gpt-3.5-turbo" if config["provider"] == "openai" else config["deployment_name"],
